...
export async function GET(req: NextRequest) {
  ...
  for (const s of SECTIONS) {
    ...
    result[s] = row
      ? { view: row.can_view, create: row.can_create ?? false, edit: row.can_edit, delete: row.can_delete }
...
export async function PUT(req: NextRequest) {
  ...
  const { profile = 'Standard', section, can_view, can_create, can_edit, can_delete } = await req.json()
  const supabase = createServerSupabase()
  const tid = getTenantId()

  const { error } = await supabase.from('role_permissions').upsert(
    { tenant_id: tid, profile, section, can_view, can_create: can_create ?? false, can_edit, can_delete },
