...
export async function POST(req: NextRequest) {
  ...
  const created = { distributors: 0 }
  const existing = { distributors: 0 }
  ...
  const [{ data: allDistricts }, { data: allTalukas }, { data: allVillages }] = await Promise.all([
  ...
  // ── Fetch existing distributors (idempotency) ─────────────────────────────
  const { data: existingDists } = await supabase.from('distributors').select('id, name').eq('tenant_id', tid)
  const existingMap = new Map(existingDists?.map(d => [d.name.toLowerCase(), d.id]) ?? [])
  ...
  for (const r of rows) {
    ...
    // Idempotency: skip if already exists in DB
    if (existingMap.has(r.name.toLowerCase())) { existing.distributors++; continue }
    ...
    let taluka_id: string | null = null
    ...
    if (r.district) {
      ...
      district_id = dist.id; state_id = dist.state_id
    }
    if (r.taluka) {
      if (!district_id) { skipped.push({ row: r.rowNum, reason: 'Taluka specified without District' }); continue }
      taluka_id = talukaMap.get(`${district_id}|${r.taluka.toLowerCase()}`) ?? null
      if (!taluka_id) { skipped.push({ row: r.rowNum, reason: `Taluka "${r.taluka}" not found in District "${r.district}"` }); continue }
    }
    if (r.village) {
      if (!taluka_id) { skipped.push({ row: r.rowNum, reason: 'Village specified without Taluka' }); continue }
  ...
  if (toInsert.length > 0) {
    const { data: nd, error } = await supabase.from('distributors').insert(toInsert).select('id')
    if (error) return NextResponse.json({ error: `Distributors: ${error.message}` }, { status: 500 })
    created.distributors = nd?.length ?? 0
  }
