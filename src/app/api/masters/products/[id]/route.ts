...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.category_id && body.subcategory_id) {
    ...
    const { data: sub } = await supabase
      .from('product_subcategories').select('category_id').eq('id', body.subcategory_id).single()
  ...
  const { data, error } = await supabase
    .from('products').update(body).eq('id', params.id).eq('tenant_id', getTenantId()).select().single()
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase
    .from('products').delete().eq('id', params.id).eq('tenant_id', getTenantId())
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
